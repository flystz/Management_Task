<x-app-layout>
  ...
  <div class="container mx-auto p-10">
    ...
    <!-- Tambahkan ini di Blade template (misalnya, tasks/index.blade.php) -->
    @if(Auth::user()->usertype === 'admin')
    <!-- add task -->
    <div class="mb-10">
      <a href="{{ route('tasks.create', $tasklist->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        Add Task
      </a>
        ...
        <tbody class="text-gray-700 text-sm font-light">
          ...
          <tr class="border-b border-gray-300 hover:bg-gray-100 transition-all duration-150 ease-in-out">
            ...
            <td class="py-3 px-6 text-center">
              <div class="flex items-center justify-center space-x-2"> <!-- Gunakan div sebagai container dengan flexbox -->
                <!-- Form untuk Memindahkan Task -->
                <form action="{{ route('moveTask', $task->id) }}" method="POST" class="m-0">
                  @csrf
                  <select name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full block" onchange="this.form.submit()">
      ...
      <table class="min-w-full bg-white border border-gray-300 rounded-lg">
        ...
        <tbody class="text-gray-700 text-sm font-light">
          ...
          <tr class="border-b border-gray-300 hover:bg-gray-100 transition-all duration-150 ease-in-out">
            ...
            <td class="py-3 px-6 text-center">
              <div class="flex items-center justify-center space-x-2"> <!-- Tambahkan Flex container dengan pengaturan alignment -->
      ...
      <table class="min-w-full bg-white border border-gray-300 rounded-lg">
        ...
        <tbody class="text-gray-700 text-sm font-light">
          ...
          <tr class="border-b border-gray-300 hover:bg-gray-100 transition-all duration-150 ease-in-out">
            ...
            <td class="py-3 px-6 text-center">
              <div class="flex items-center justify-center space-x-2"> <!-- Flex container -->
                <!-- Form untuk Mengubah Status Task -->
                <form action="{{ route('moveTask', $task->id) }}" method="POST" class="m-0">
                  @csrf
                  <select name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  w-full" onchange="this.form.submit()">
                    <option value="todo" {{ $task->status == 'todo' ? 'selected' : '' }}>Todo</option>
                    <option value="on_progress" {{ $task->status == 'on_progress' ? 'selected' : '' }}>On Progress</option>
                    <option value="complete" {{ $task->status == 'complete' ? 'selected' : '' }}>Complete</option>
                  </select>
